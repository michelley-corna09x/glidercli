...
Registry is loaded from (first found): `GLIDER_BROWSERS_REGISTRY` env, `~/.cursor/registry/browsers.json`, `~/.glider/config/browsers-registry.json`. Keys are predefined (e.g. `arc`, `brave`, `chrome`, `edge`, `opera`, `vivaldi`, `duckduckgo`, `chromium`). Edit the registry to add or change paths.
...
| Configure | `~/.glider/config/browser.json` (use key or name/path) and/or `GLIDER_BROWSER*`. Registry: `~/.cursor/registry/browsers.json` or `GLIDER_BROWSERS_REGISTRY`. |
